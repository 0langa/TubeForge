...
public static class DownloadSourceIdentityTests
{
    ...
    public static void RoundTripsDirectAndAdaptiveSelections()
    {
        ...
        Assert.True(CaptionEmbedSelectionSet.TryCreate(
            [caption, new CaptionEmbedSelection("de", IsAutoGenerated: true)],
            out var captions));
        var multiCaptionValue = DownloadSourceIdentity.Create(
            videoId,
            401,
            140,
            OutputProfile.H264AacMp4,
            captions);
        Assert.True(DownloadSourceIdentity.TryParse(multiCaptionValue, out var multiCaptioned));
        Assert.Equal(captions, multiCaptioned.Captions);
        Assert.True(multiCaptioned.Caption is null);
        Assert.Equal(
            $"dQw4w9WgXcQ:401+140@{OutputProfile.H264AacMp4.Identity}~m.en-US,a.de",
            multiCaptionValue);
        ...
        foreach (var value in new[]
                 {
                     ...
                     "dQw4w9WgXcQ:18~m.en,m.en", "dQw4w9WgXcQ:18~m.en,",
